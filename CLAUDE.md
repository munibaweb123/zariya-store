...
- **Order numbers are `RR-YYMMDD-XXXX`** (e.g. `RR-260803-K7QF`, prefix updated from `ZR-` when the brand was renamed to Range Ronaq — any pre-existing `ZR-` orders keep their original numbers), generated by `lib/orders.ts`'s `generateOrderNumber()` *before* insert — not derived from the database id, so it never reveals total order count. The `orderNumber` column has a unique constraint; callers must catch a collision and retry (up to 3 times) rather than assume uniqueness.
...
Established in `frontend/07-order-confirmation-page`:

- **All seven storefront pages are now built** — home, category, product detail, cart, checkout, and order confirmation. The full guest order flow (browse → cart → checkout → confirmation) is complete end-to-end; only `/our-story` (static content, lowest priority) remains from CLAUDE.md's Pages list.
- **A page's server-vs-client split follows the data, not a blanket rule.** `/order/[orderNumber]` fetches real Postgres data, so — unlike `/cart` and `/checkout`, whose entire content was client-only cart state — it's a normal Server Component, with exactly one tiny `'use client'` leaf (`ClearCartOnConfirmation`) for the one genuinely client-side side effect (clearing the cart). Check what the page's content actually depends on before defaulting to whole-page client, in either direction.
- **Clearing client state on mount must be gated on hydration completing first, or it can silently lose the race.** `ClearCartOnConfirmation` waits for `useCart().hasHydrated` before calling `clearCart()` — `CartProvider`'s `rehydrate()` reads the pre-order cart back from `localStorage` on mount, and if that finishes *after* an unconditional clear, it repopulates the cart with the old contents, undoing the clear. Verified directly against a real pre-seeded `localStorage` cart, not just reasoned about. Any future "clear/reset persisted client state on this page" feature must follow the same hydration-gated pattern.
...
- **Brand name:** `Range Ronaq` (renamed from `ZARIYA`; domain not finalised).
